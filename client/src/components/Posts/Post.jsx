...
import { LikeFill, LikeOutline, Delete, More, EnterPost } from "../../images/Icons";
...
function Post({ post, currentId, setCurrentId }) {
    ...
    return (
        <div className="bg-whit overflow-hidden rounded-xl h-full flex flex-col min-h-[30rem] shadow-xl hover:shadow-lg active:shadow duration-200">
            ...
            <div className="flex flex-col justify-between grow pb-3">
                <Link to={`/post/${post._id}`} className="grow" >
                    <div className="relative duration-300 h-full">
                        <div className="active:scale-80 absolute opacity-0 hover:opacity-100 w-full h-full bg-[rgba(0,0,0,0.1)] shadow-0 duration-300 text-white backdrop-blur-sm">
                            <div className="w-[200%] h-full translate-x-[-50%] hover:translate-x-0 duration-500"><EnterPost /></div>
                        </div>
                ...
                <div className="w-full relative mb-3 mt-6 h-5">
                    <button
                        ...
                        className={`absolute top-[50%] translate-y-[-50%] left-0 flex flex-row ${!user?.result ? "text-gray-400" : "text-blue-600"
                            } font-semibold mx-6`}
                    >
                        <Likes />
                    </button>
