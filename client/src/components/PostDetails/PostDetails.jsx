...
import { Back, LikeFill } from "../../images/Icons";
...
const PostDetails = () => {
  ...
  if (!loading && post.name) {
    return (
      <>
        <div className="max-w-6xl mx-auto">
          <Link
            to={"/"}
            className="w-fit mx-2 px-4 py-2 hover:opacity-80 duration-300 rounded-md hover:rounded-3xl bg-gray-100 border shadow-md flex items-center justify-center font-bold"
          >
            <Back />
          </Link>
        </div>
        ...
        <div className="relative flex gap-10 max-sm:flex-col-reverse justify-between items-center border p-3 max-w-6xl max-sm:my-5 rounded-lg mx-auto m-10 shadow-lg">
          <div className="relative flex flex-col gap-3 ml-10 max-sm:mx-5 h-full">
            <section className="text-xl font-bold">{post.title}</section>
            <section className="flex gap-3 font-bold text-gray-600 my-2">
              {post.tags?.map((t) => (
                <div
                  key={t}
                  className="px-3 bg-gray-200 rounded-full inline shadow-md hover:border-black duration-300 border cursor-pointer"
                >
                  {" "}
                  #{t}
                </div>
